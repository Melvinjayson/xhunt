...
import {
  ...
  X, ChevronRight, Play, Users, Zap, Sparkles,
...
function AvatarCircle({ name, url, size = 36 }: { name: string; url: string | null; size?: number }) {
  return url ? (
    ...
    <div style={{
      ...
      background: 'linear-gradient(135deg,#27e07d22,#22d3ee22)',
      border: '1px solid rgba(39,224,125,.25)',
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px 10px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{
            ...
            color: isLive ? T.live : T.amber, textTransform: 'uppercase',
          }}>
      ...
      <div style={{ padding: '0 16px 14px' }}>
        <div style={{ fontSize: 16, fontWeight: 700, color: T.txt, marginBottom: 4, lineHeight: 1.3 }}>
          {session.title}
        </div>
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: T.txt, lineHeight: 1.2 }}>
      ...
      {/* Caption */}
      {post.caption && (
        <p style={{ fontSize: 14, color: T.txt, lineHeight: 1.5, marginBottom: 10 }}>
          {post.caption}
        </p>
      )}
      ...
      {(post.mission_title || xp) && (
        <div style={{
          ...
          {xp && (
            <span style={{ fontSize: 12, fontWeight: 700, color: T.amber, whiteSpace: 'nowrap' }}>
              +{xp} XP
            </span>
          )}
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <button
          ...
          style={{
            ...
            color: T.muted, fontSize: 13, fontWeight: 600, padding: 0,
          }}
        >
          <Flame size={16} />
...
function GoLiveModal({ onClose, onCreated }: { onClose: () => void; onCreated: (id: string) => void }) {
  ...
  async function handleStart() {
    ...
    try {
      const res = await fetch('/api/live/start', {
        ...
        body: JSON.stringify({ title: title.trim(), description: description.trim() || undefined, is_pro_only: isProOnly }),
      });
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
      onClick={onClose}
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 100, display: 'flex', alignItems: 'flex-end' }}
    >
      <motion.div
        initial={{ y: '100%' }}
        animate={{ y: 0 }}
        ...
        style={{
          ...
          border: `1px solid ${T.line2}`, padding: '24px 20px 32px',
        }}
      >
        ...
        <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: T.muted, marginBottom: 6, letterSpacing: '0.04em' }}>
          SESSION TITLE *
        </label>
        <input
          value={title}
          onChange={e => setTitle(e.target.value)}
          placeholder="What are you doing live?"
          maxLength={80}
          style={{
            width: '100%', padding: '12px 14px', borderRadius: 12,
            background: T.elev, border: `1px solid ${T.line2}`, color: T.txt,
        ...
        <textarea
          ...
          placeholder="Brief description of what viewers will experience..."
          rows={3}
          maxLength={200}
          style={{
            width: '100%', padding: '12px 14px', borderRadius: 12,
            background: T.elev, border: `1px solid ${T.line2}`, color: T.txt,
            fontSize: 14, outline: 'none', marginBottom: 14, resize: 'none', boxSizing: 'border-box',
          }}
        />
        ...
        >
          ...
          <div style={{
            ...
            background: isProOnly ? T.amber : T.elev,
            border: `1px solid ${isProOnly ? T.amber : T.line2}`,
            display: 'flex', alignItems: 'center',
            padding: 3, transition: 'background .2s',
          }}>
            <motion.div
              animate={{ x: isProOnly ? 18 : 0 }}
        ...
        {error && (
          <p style={{ fontSize: 13, color: T.red, marginBottom: 14 }}>{error}</p>
        )}
        ...
        <button
          ...
          style={{
            ...
            background: (!loading && title.trim()) ? T.live : T.elev,
            color: (!loading && title.trim()) ? '#fff' : T.dim,
            fontWeight: 700, fontSize: 15, cursor: loading || !title.trim() ? 'not-allowed' : 'pointer',
  ...
  async function handleShare() {
    ...
    try {
      await fetch('/api/timeline/post', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ post_type: 'moment', caption: caption.trim() || undefined }),
      });
      onCreated();
      onClose();
    } finally {
      ...
      >
        ...
        <textarea
          ...
          style={{
            width: '100%', padding: '12px 14px', borderRadius: 12,
            background: T.elev, border: `1px solid ${T.line2}`, color: T.txt,
            fontSize: 14, outline: 'none', marginBottom: 16, resize: 'none', boxSizing: 'border-box',
          }}
        />
        <button
          onClick={handleShare}
          ...
          style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
            width: '100%', padding: '14px', borderRadius: 14, border: 'none',
            background: T.green, color: '#070d0e', fontWeight: 700, fontSize: 15,
...
function UpgradeSheet({ reason, onClose }: { reason: string; onClose: () => void }) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
      onClick={onClose}
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 110, display: 'flex', alignItems: 'flex-end' }}
    >
      <motion.div
        initial={{ y: '100%' }}
        animate={{ y: 0 }}
        ...
        style={{
          width: '100%', maxWidth: 430, margin: '0 auto',
          background: T.panel, borderRadius: '24px 24px 0 0',
          border: `1px solid ${T.line2}`, padding: '28px 20px 36px',
          textAlign: 'center',
        }}
      >
        ...
        <div style={{ display: 'flex', gap: 12, flexDirection: 'column' }}>
          <button
            ...
            style={{
              ...
              color: '#070d0e', fontWeight: 700, fontSize: 15,
            }}
          >
          ...
          <Link
            ...
            style={{
              ...
              border: `1px solid ${T.line2}`, color: T.muted,
...
export default function TimelinePage() {
  ...
  function handleGoLive() {
    if (tier !== 'pro') {
      setUpgradeReason('Go Live lets you host real-time mission experiences for your community. Upgrade to Pro to unlock hosting.');
    } else {
      setShowGoLive(true);
    }
  }
  ...
  function handleJoinLive(session: LiveSession) {
    if (session.is_pro_only && tier !== 'pro') {
      setUpgradeReason('This is a Pro-only live session. Upgrade to Pro to watch and participate in exclusive live experiences.');
    } else {
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            background: T.live, color: '#fff', fontWeight: 700, fontSize: 13,
          }}
        >
          <motion.div
            animate={{ opacity: [1, 0.4, 1] }}
        ...
        {FILTERS.map(f => (
          <button
            ...
            style={{
              ...
              background: filter === f ? T.green : T.elev,
              color: filter === f ? '#070d0e' : T.muted,
              fontWeight: filter === f ? 700 : 500, fontSize: 13, whiteSpace: 'nowrap',
            }}
          >
            {f === 'Live' && '🔴 '}{f}
          </button>
            ...
            {isEmpty && (
              <div style={{ textAlign: 'center', padding: '3rem 1rem' }}>
                ...
                <p style={{ fontSize: 13, color: T.muted, marginBottom: 20 }}>
                  {filter === 'Live'
                    ? 'Pro users can go live and host mission experiences in real time.'
                    : 'Complete missions and share your experiences here.'}
                </p>
                <button
                  ...
                  style={{
                    ...
                    background: T.green, color: '#070d0e', fontWeight: 700, fontSize: 14,
                  }}
                >
      ...
      {!isEmpty && (
        <motion.button
          ...
          style={{
            ...
            background: `linear-gradient(135deg,${T.green},${T.ai})`,
            boxShadow: '0 4px 20px rgba(39,224,125,.35)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}
        >
          <Plus size={22} color="#070d0e" strokeWidth={2.5} />
        </motion.button>
      )}
