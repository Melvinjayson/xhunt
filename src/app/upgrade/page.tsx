...
function FeatureRow({ text, included }: { text: string; included: boolean }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
      <div style={{ width: 20, height: 20, borderRadius: '50%', flexShrink: 0, background: included ? 'rgba(34,255,170,.15)' : 'rgba(255,255,255,.04)', border: `1px solid ${included ? 'rgba(34,255,170,.3)' : 'rgba(255,255,255,.07)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {included && <Check size={11} strokeWidth={3} style={{ color: t.accent }} />}
      </div>
      <span style={{ fontSize: 13, color: included ? t.txt : t.txtFaint }}>{text}</span>
    </div>
  );
}
    ...
    <div style={{ minHeight: '100vh', background: `radial-gradient(800px 600px at 50% 0%, rgba(34,255,170,.07), transparent 60%), ${t.bg}`, color: t.txt, fontFamily: 'var(--font-onest), system-ui' }}>
      <div style={{ maxWidth: 430, margin: '0 auto', padding: '0 20px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 52, paddingBottom: 28 }}>
          <button onClick={() => router.back()} style={{ width: 38, height: 38, borderRadius: '50%', background: t.card, border: `1px solid rgba(255,255,255,.07)`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
        ...
        {justUpgraded && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div style={{ width: 36, height: 36, borderRadius: 12, background: 'rgba(34,255,170,.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Check size={18} style={{ color: t.accent }} strokeWidth={2.5} />
            </div>
            <div>
              <p style={{ margin: 0, fontSize: 14, fontWeight: 800, color: t.txt }}>Welcome to Pro!</p>
              <p style={{ margin: '2px 0 0', fontSize: 12, color: t.txtDim }}>Your subscription is now active. Enjoy unlimited AI access.</p>
            </div>
          </motion.div>
        )}
        ...
        {loading ? (
          <div style={{ display: 'flex', justifyContent: 'center', padding: '80px 0' }}>
            <Loader2 size={28} style={{ color: t.accent, animation: 'spin 0.9s linear infinite' }} strokeWidth={2} />
          </div>
        ) : (
          <>
            {/* Current plan badge */}
            {tierInfo && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                style={{ borderRadius: 18, padding: '16px 18px', marginBottom: 24, background: isTrial ? 'rgba(109,93,253,.07)' : isPro ? 'rgba(34,255,170,.07)' : t.card, border: `1px solid ${isTrial ? 'rgba(109,93,253,.18)' : isPro ? 'rgba(34,255,170,.18)' : 'rgba(255,255,255,.07)'}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <p style={{ margin: '0 0 3px', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', color: isTrial ? t.ai : isPro ? t.accent : t.txtFaint }}>Current Plan</p>
                  ...
                  {isTrial && tierInfo.trialEndsAt && (
                    <p style={{ margin: '3px 0 0', fontSize: 11, color: t.txtFaint, display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <div style={{ width: 42, height: 42, borderRadius: 14, background: isTrial ? 'rgba(109,93,253,.1)' : isPro ? 'rgba(34,255,170,.1)' : 'rgba(255,255,255,.04)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {isPro ? <Star size={20} style={{ color: t.accent }} strokeWidth={1.8} /> : isTrial ? <Sparkles size={20} style={{ color: t.ai }} strokeWidth={1.8} /> : <Zap size={20} style={{ color: t.txtFaint }} strokeWidth={1.8} />}
                </div>
              </motion.div>
            )}
            ...
            {isTrial && (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                ...
                <p style={{ margin: 0, fontSize: 13, color: t.txtDim, lineHeight: 1.6 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
                style={{ borderRadius: 20, padding: '18px 18px 20px', background: t.card, border: `1px solid ${isFree && !tierInfo?.hasUsedTrial ? 'rgba(255,255,255,.15)' : 'rgba(255,255,255,.07)'}` }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                  <div>
                    <p style={{ margin: 0, fontSize: 17, fontWeight: 800, color: t.txt }}>Free</p>
                    <p style={{ margin: '2px 0 0', fontSize: 11, color: t.txtFaint }}>Always</p>
                  </div>
                ...
                <div style={{ padding: '18px 18px 20px', background: 'rgba(34,255,170,.02)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <p style={{ margin: 0, fontSize: 22, fontWeight: 900, color: t.accent }}>$12<span style={{ fontSize: 13, fontWeight: 600, color: t.txtDim }}>/mo</span></p>
                    </div>
                  </div>
                  <p style={{ margin: '0 0 16px', fontSize: 11, color: t.txtFaint }}>Full AI power, no limits</p>
                  ...
                  {!isPro ? (
                    <motion.button whileTap={{ scale: 0.98 }} onClick={() => void handleUpgrade()} disabled={upgrading}
                      style={{ width: '100%', height: 50, borderRadius: 999, border: 0, cursor: upgrading ? 'default' : 'pointer', marginTop: 16, fontFamily: 'inherit', fontSize: 14, fontWeight: 700, background: t.accent, color: t.bg, boxShadow: '0 4px 20px rgba(34,255,170,.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: upgrading ? 0.7 : 1 }}>
                    ...
                    <div style={{ marginTop: 16, padding: '10px 14px', borderRadius: 12, background: 'rgba(34,255,170,.08)', border: '1px solid rgba(34,255,170,.2)', display: 'flex', alignItems: 'center', gap: 8 }}>
                      <Check size={15} style={{ color: t.accent }} strokeWidth={2.5} />
                      <span style={{ fontSize: 13, fontWeight: 600, color: t.accent }}>Active Pro plan</span>
                    </div>
                  )}
                </div>
              </motion.div>
            ...
            <p style={{ textAlign: 'center', fontSize: 11, color: t.txtFaint, marginTop: 24 }}>
  ...
  return (
    <Suspense fallback={
      <div style={{ minHeight: '100vh', background: t.bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: 28, height: 28, border: `2px solid ${t.accent}`, borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.9s linear infinite' }} />
      </div>
    }>
      <UpgradePageInner />
    </Suspense>
