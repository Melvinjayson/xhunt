  ...
  return (
    <main className="consumer-app" style={{ background: t.bg, minHeight: '100dvh', paddingBottom: '5.5rem', color: t.txt }}>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 40, background: 'rgba(5,8,22,.94)', backdropFilter: 'blur(20px)', borderBottom: `1px solid ${LINE}`, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={() => router.back()} style={{ width: 36, height: 36, borderRadius: '50%', background: t.card, border: `1px solid ${LINE}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '0 16px' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'linear-gradient(135deg,rgba(34,255,170,1),rgba(109,93,253,1))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 900, color: t.bg, flexShrink: 0 }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <p style={{ margin: 0, fontSize: 10, color: t.txtDim }}>Hunter Score</p>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
          style={{ marginBottom: 16, borderRadius: 20, background: t.surface, border: `1px solid ${LINE}`, overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px 10px', borderBottom: `1px solid ${LINE}` }}>
            <p style={{ margin: 0, fontSize: 10, fontWeight: 700, color: t.txtDim, textTransform: 'uppercase', letterSpacing: '.08em' }}>Participation Wallet</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)' }}>
            {[
              ...
              <div key={label} style={{ padding: '14px 12px', borderRight: i < 2 ? `1px solid ${LINE}` : 'none', textAlign: 'center' }}>
                <p style={{ margin: 0, fontSize: 20, fontWeight: 900, color, letterSpacing: '-.03em', lineHeight: 1 }}>{value}</p>
                <p style={{ margin: '3px 0 0', fontSize: 9.5, color: t.txtDim, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.04em' }}>{label}</p>
                <p style={{ margin: '1px 0 0', fontSize: 9, color: t.txtDim }}>{sub}</p>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 16 }}>
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
              style={{ padding: '14px 12px', borderRadius: 16, background: t.card, border: `1px solid ${LINE}`, textAlign: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
          <div style={{ padding: '14px 16px', borderRadius: 16, background: t.card, border: `1px solid ${LINE}`, display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        {nextTier && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            style={{ marginBottom: 20, padding: '16px', borderRadius: 18, background: t.surface, border: `1px solid ${LINE}` }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          {earnedBadges.length > 0 ? (
            ...
            <div style={{ padding: '20px', borderRadius: 16, background: t.card, border: `1px solid ${LINE}`, textAlign: 'center', marginBottom: 12 }}>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                {lockedBadges.slice(0, 6).map(badge => (
                  <div key={badge.id} style={{ padding: '12px 8px', borderRadius: 14, background: t.surface, border: `1px solid ${LINE}`, textAlign: 'center', opacity: 0.5 }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ borderRadius: 16, background: t.card, border: `1px solid ${LINE}`, overflow: 'hidden' }}>
              {pendingRewards.map(({ hunt, record }, i) => (
                <div key={hunt.id} style={{ padding: '12px 14px', borderBottom: i < pendingRewards.length - 1 ? `1px solid ${LINE}` : 'none', display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ margin: '2px 0 0', fontSize: 11, color: t.warning, fontWeight: 600 }}>
                      {record.status === 'ai_reviewing' ? '🤖 AI Reviewing' : record.status === 'manual_review' ? '👁 In Review' : '📬 Submitted'}
                    </p>
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <p style={{ margin: 0, fontSize: 14, fontWeight: 800, color: t.warning }}>~${estimateCashReward(hunt.cashReward, hunt.difficulty, hunt.missionType)}</p>
                    <p style={{ margin: '2px 0 0', fontSize: 10, color: t.txtFaint }}>pending</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ borderRadius: 16, background: t.card, border: `1px solid ${LINE}`, overflow: 'hidden' }}>
              {[...completedHunts].reverse().slice(0, 5).map((h, i) => (
                <div key={`${h.huntId}-${i}`} style={{ padding: '12px 14px', borderBottom: i < 4 ? `1px solid ${LINE}` : 'none', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
          ...
          <div style={{ width: 48, height: 48, borderRadius: 14, background: 'linear-gradient(135deg,rgba(34,255,170,1),rgba(109,93,253,1))', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
